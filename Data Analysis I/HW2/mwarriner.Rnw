...
\begin{enumerate}
  ...
  \begin{enumerate}
    ...
    \begin{enumerate}
      ...
      \begin{figure}[H]
      <<warning=F, size="scriptsize", message=F, fig.dim = c(6,3)>>=
      ...
      p = 0.5 #our hypothesis probability
      
      #Create tibble of associated probabilities of each observation under the binomial distribution
      ggdat <- tibble(x = 1:n, 
                      p = dbinom(1:n,size = n, p = p))
      
      ggplot(data = ggdat, aes(x = x, y = p)) +
        geom_linerange(aes(ymin = 0, ymax = p), linewidth = 3) + 
        theme_bw() + 
        geom_hline(yintercept = 0) + 
        ylab('density') +
        labs(subtitle = 'n = 15, p = 0.50') +
        xlim(0,15)
      ...
      alpha = 0.05 #intended alpha
      qbinom(p = alpha, size = n, prob = p) #inverse cdf to find 5th percentile observation
      ...
      # find the associated probability of observing 4 or fewer from our null distribution
      ...
      pbinom(q = 3, size = n, prob = p) # Test out 3
    ...
    \begin{enumerate}
      ...
      # create tibble of gaussian densities
      gaussian_dat <- tibble(x = seq(from = -5,
                                     to = 5, 
                                     length.out = 10000), 
                             d = dnorm(seq(from = -5, 
                                           to = 5, 
      ...
      ggplot(data = gaussian_dat, aes(x = x, y = d)) + geom_line() + 
        theme_bw() + 
        xlab('Z') + 
        ylab('density') + 
        geom_hline(yintercept = 0)
      ...
      fifth_percentile <- qnorm(p = alpha)
...
library(tidyverse)
@
<<exactdist, echo=F, eval=F, size="scriptsize", warning=F, message=F>>=
# Do R simulations, compute and store x bar
set.seed(1313)
R <- 10000
xbar <- rep(NA, R)
n <- 10       # sample size for the simulation
lambda <- 2   # lambda for the simulation
for(i in 1:10000){
  x <- rexp(n=n, rate=1/lambda)  # Generate Data
  xbar[i] <- mean(x)             # Compute the mean
}
# Create a tibble with the sampling distributions for comparison
ggdat.sampdists <- tibble(x = seq(0, 7, length.out=1000)) |>
  mutate(gamma.pdf = dgamma(x, shape = n, rate = (n/lambda)),
         clt.pdf = dt((x-lambda)/(sd(x)/sqrt(n)), df=n-1))
ggplot()+
  geom_histogram(data=tibble(xbar=xbar),
                 aes(x=xbar, y=after_stat(density)),
                 breaks=seq(0,7,0.25),
                 color="lightgrey")+
  geom_line(data=ggdat.sampdists,
            aes(x=x, y=gamma.pdf, color="Gamma"))+
  geom_line(data=ggdat.sampdists,
            aes(x=x, y=clt.pdf, color="T Distribution (CLT)"))+
  geom_hline(yintercept=0)+
  theme_bw()+
  xlab(bquote(bar(x)))+
  ylab("Density")+
...
@
  \begin{figure}[H]
    \centering
<<echo=F, warning=F, message=F, fig.dim=c(5,2.5)>>=
<<exactdist>>
@
...
(low_percentile_value <- qgamma(p = low_percentile, shape = n, rate = n/lambda))

(p_low_percentile <- mean(xbar<low_percentile_value))


(high_percentile_value <- qgamma(p = high_percentile, shape = n, rate = n/lambda))

(p_high_percentile <- mean(xbar>high_percentile_value))

(p_low_percentile + p_high_percentile)
...
(p_between <- mean((xbar>low_percentile_value) & (xbar<high_percentile_value)))
...
(low_percentile_value <- qnorm(p = low_percentile, mean = lambda, sd = lambda/sqrt(n)))

(p_low_percentile <- mean(xbar<low_percentile_value))


(high_percentile_value <- qnorm(p = high_percentile, mean = lambda, sd = lambda/sqrt(n)))
...
correct_pearson <- rep(NA, 10000)
correct_wilson <- rep(NA, 10000)

for(i in 1:10000){
  
random_binom <- rbinom(n = 15, size = 1, prob = 0.10)

successes <- sum(random_binom)
  
conf_ints <- binom.confint(x = successes, n = 15, methods = c("exact", "wilson"))

pearson_ci <- c(conf_ints$lower[1], conf_ints$upper[1])

wilson_ci <- c(conf_ints$lower[2], conf_ints$upper[2])

correct_pearson[i] <- ifelse(0.1 >= pearson_ci[1] && 0.1 <= pearson_ci[2], 1, 0)

correct_wilson[i] <- ifelse(0.1 >= wilson_ci[1] && 0.1 <= wilson_ci[2], 1, 0) 
  
}

mean(correct_pearson)
...
random_binom <- rbinom(n = 1000, size = 1, prob = 0.10)

successes <- sum(random_binom)
  
conf_ints <- binom.confint(x = successes, n = 1000, methods = c("exact", "wilson"))
...
random_binom <- rbinom(n = 15, size = 1, prob = 0.50)
...
correct_pearson[i] <- ifelse(0.5 >= pearson_ci[1] && 0.5 <= pearson_ci[2], 1, 0)

correct_wilson[i] <- ifelse(0.5 >= wilson_ci[1] && 0.5 <= wilson_ci[2], 1, 0) 
...
random_binom <- rbinom(n = 5, size = 1, prob = 0.50)

successes <- sum(random_binom)
  
conf_ints <- binom.confint(x = successes, n = 5, methods = c("exact", "wilson"))
...
<<size="scriptsize", warning=F, message=F>>=
dat <- c(4.14, 1.23, 3.28, 4.58, 1.33, 1.93, 7.97, 7.99, 15.30, 14.00, 4.31, 14.87, 
         28.08, 11.18, 12.56, 2.00, 1.26, 149.02, 0.27, 1.45, 3.13, 0.76, 15.02, 
         5.36, 32.60, 22.08, 261.17, 6.61, 2.43, 5.02, 1.16, 5.94, 3.36, 15.91)
@
  \begin{enumerate}
